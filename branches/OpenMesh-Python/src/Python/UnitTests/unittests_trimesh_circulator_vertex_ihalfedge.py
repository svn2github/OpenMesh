...
class TriMeshCirculatorVertexIHalfEdge(unittest.TestCase):
    ...
    def test_vertex_incoming_halfedge_without_holes_increment(self):
        ...
        # self.assertEqual(mesh.face_handle(heh).idx(), 1)
        heh = vih_it.__next__()
        self.assertEqual(heh.idx(), 7)
        # self.assertEqual(mesh.face_handle(heh).idx(), 2)
        heh = vih_it.__next__()
        self.assertEqual(heh.idx(), 0)
        # self.assertEqual(mesh.face_handle(heh).idx(), 0)
        heh = vih_it.__next__()
        self.assertEqual(heh.idx(), 3)
        # self.assertEqual(mesh.face_handle(heh).idx(), 3)
    ...
    def test_vertex_incoming_halfedge_boundary_increment(self):
        ...
        # TODO self.assertEqual(mesh.face_handle(heh).idx(), 3)
        heh = vih_it.__next__()
        self.assertEqual(heh.idx(), 2)
        # TODO self.assertEqual(mesh.face_handle(heh).idx(), 0)
        heh = vih_it.__next__()
        self.assertEqual(heh.idx(), 5)
        # TODO self.assertEqual(mesh.face_handle(heh).idx(), -1)
        self.assertRaises(StopIteration, vih_it.__next__)
